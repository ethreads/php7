...
struct _zend_class_entry {
	...
	zend_string *name;                  /* 类名，统一为小写 */
	struct _zend_class_entry *parent;   /* 父类 */
	int refcount;
	uint32_t ce_flags;                  /* 类的掩码 */

	int default_properties_count;       /* 普通属性数，public、private */
	...
	HashTable constants_table;          /* 常量哈希表，constant定义的 */
    /* 构造函数、析构函数、魔术方法的指针 */
	...
	/* handlers 自定义的钩子函数，通常是定义内部类时使用，可以灵活的进行一些个性化的操作 */
	zend_object* (*create_object)(zend_class_entry *class_type);
	zend_object_iterator *(*get_iterator)(zend_class_entry *ce, zval *object, int by_ref);
	int (*interface_gets_implemented)(zend_class_entry *iface, zend_class_entry *class_type); /* a class implements this interface */
	union _zend_function *(*get_static_method)(zend_class_entry *ce, zend_string* method);

	/* serializer callbacks 实现序列化的钩子函数 */
