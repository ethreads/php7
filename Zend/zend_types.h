...
typedef union _zend_value {
	zend_long         lval;				/* long value int整形 */
	...
	union {
		struct {
			ZEND_ENDIAN_LOHI_4(
				...
				zend_uchar    type_flags,    /* 类型掩码。不同类型会有不同的几种属性，内存管理时使用 */
				zend_uchar    const_flags,
				zend_uchar    reserved)	    /* call info for EX(This) zend执行流程使用 */
		} v;
		uint32_t type_info;
	} u1;
	union {
...
typedef struct _Bucket {
	zval              val;              /* 存储的具体value，这里嵌入一个zval，而不是指针 */
...
struct _zend_array {
	...
	Bucket           *arData;           /* bucket数组,指向存储元素数组的第一个Bucket */
...
/*
 * & 首先会创建一个 zend_reference 结构，其内嵌了一个zval，这个zval的value指向原来zval的value
 * 然后将原zval的类型修改为IS_REFERENCE，原zval的value指向新 创建的 zend_reference 结构。
 * PHP中的 引用只可能有一层 ，不会出现一个引用指向另外一个引用的情况
 */
