...
ZEND_API zend_executor_globals executor_globals;    /* 记录着当前请求全部的信息,宏EG操作的就是这个结构 */
#endif
...
void zend_compile_func_decl(znode *result, zend_ast *ast) /* {{{ */
{
	...
	zend_op_array *orig_op_array = CG(active_op_array);     /* 保存当前正在编译的zend_op_array */
	zend_op_array *op_array = zend_arena_alloc(&CG(arena), sizeof(zend_op_array));  /* 重新为函数生成一个新的zend_op_array */
	...
	CG(active_op_array) = orig_op_array;    /* 编译完成后将旧的还原 */
}
/* }}} */
