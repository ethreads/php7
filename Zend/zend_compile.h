...
struct _zend_op {
	const void *handler;    /* 指令执行handler (zend_vm_def.h) */
	znode_op op1;           /* 操作数1 */
	znode_op op2;           /* 操作数2 */
	znode_op result;        /* 返回值 */
...
typedef struct _zend_arg_info {
	...
	zend_bool is_variadic;          /* 是否为可变参数，即...用法 */
} zend_arg_info;
...
struct _zend_op_array {
	/* Common elements 普通函数或类成员方法对应的opcades快速访问时使用的字断 */
	zend_uchar type;
	...
	/* PHP代码里定义的变量数：op_type为IS_CV的变量（不含IS_TMP_VAR、IS_VAR），编译前为0，发现一个+1 */
	int last_var;
	...
	/* static variables support 静态变量符号表:通过static声明的 */
...
union _zend_function {
	...
	struct {
		...
		uint32_t fn_flags;
		zend_string *function_name;
		zend_class_entry *scope;            /* 成员方法所属类，面向对象实现中使用 */
		union _zend_function *prototype;
		uint32_t num_args;                  /* 参数数量 */
		uint32_t required_num_args;         /* 必传参数数量 */
		zend_arg_info *arg_info;            /* 参数信息（如果函数声明了返回类型会在这个数组的-1位置） */
	} common;
...
struct _zend_execute_data {
	...
	zval                *literals;         /* cache op_array->literals 字面量数组，与func.op_array->literals相同 */
#endif
};
